...
export class RestaurantService {
  ...
  async createRestaurant(
    ...
    session?: ClientSession, // Add session as an optional parameter
    ...
    try {
      ...
      // Save the restaurant with session (if provided)
  ...
  async getRestaurantById(id: string): Promise<RestaurantResponse> {
    try {
      const restaurant = await this.restaurantModel
        .findById(id)
        .populate('manager', ' _id fullName email phoneNumber')
        .populate('menu');
  ...
  async findRestaurantByName(
    ...
    try {
      ...
      throw new BadRequestException(
        `Error finding restaurant by name: ${error.message}`
      );
    }
  } 
}
...
//   async findRestaurantByName(name: string, session?: any): Promise<Restaurant | null> {
//   try {
//     const restaurant = await this.restaurantModel
//       .findOne({ name: name })
//       .session(session)
